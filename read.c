...
object sfs_read( char *input, uint *here ) {
		...
		if ( input[*here] == '(' ) {
			if ( first_usefull_char( input + *here + 1 )[*here] == ')' ) {		/*Regarde si le prochain caractère utile est ')', si c'est le cas, on à faire à la liste vide*/
				*here +=  2;
				return nil;
			}
			else 
		...
		else if ( isspace( input[*here] ) )					/*Regarde si le caractère input[*here] est un espace, si c'est le cas on incrémente de 1 le pointeur et on relit la chaine de caractère à partir de ce pointeur*/
 		{	*here += 1;
...
object sfs_read_atom( char *input, uint *here ) {
	...
	case '+': case '-': 
		...
		{	sscanf(input + *here,"%d",&a);					/*Met dans a le nombre pointé par input + *here*/
			atom = make_integer(a);
			sscanf(input + *here + 1,"%d",&b);				/*Met dans b le nombre pointé par input + *here sans l'opérateur*/
			*here = *here + log10(b) + 2;					/*Met à jour le pointeur*/
		}
		break;
	case '0': case '1': case '2': case '3': case '4': case '5': case'6': case'7': case'8': case'9':
		sscanf( input + *here,"%d",&a );
		atom = make_integer( a );
		*here = *here + log10(a) + 1;
		break;
	case '"':
		while ( input[*here + 1] != '"' )					/*Tant que la chaine de caractère n'est pas '"', on met chaque caractère dans un tableau de char*/
		{	s1[j] = input[*here];
			j++;
			(*here)++;
		}
	...
	case '#':
		...
		case '\\' :
			...
			*here+=3;
			break;
		case 't': case'f':
			atom=make_boolean(input[(*here)+1]);
			*here+=2;
			break;
		}
		break;
	/*case '\\':
		atom=make_string(&input[(*here)+1]);
		*here+=2;*/
	default:
		sscanf(input + *here,"%s",s2);
		if ( s2[strlen(s2)-1] == ')')						/*Regarde si le dernier caractère du symbol est ')'*/
		{	atom=make_symbol(s2);						/*Si c'est le cas, il ne faut créer un symbole qu'avec s2 moins le dernier caractère ce que nous n'avons pas réussi à faire*/
			*here = *here + strlen(s2) - 1;
		}
		else
		{	atom = make_symbol(s2);
			*here = *here + strlen(s2);
...
object sfs_read_pair( char *stream, uint *i ) {
	...
	o->this.pair.car = sfs_read(stream,i);	
	if ( ')' == stream[*i] )
	{	o->this.pair.cdr = make_empty_list();
		(*i)++;
	}
	else 
		o->this.pair.cdr = sfs_read_pair(stream,i);
	return o;
}
